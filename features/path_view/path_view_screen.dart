...
import '../general_quiz/general_quiz_screen.dart';
...
class PathViewScreen extends StatefulWidget {
  ...
  final VoidCallback? onChangeCareer;
...
class _PathViewScreenState extends State<PathViewScreen> {
  ...
  Future<void> _selectTrack() async {
    await ProgressService().selectTrack(widget.college, widget.specialization);
  ...
  Future<void> _handleFinalPhaseTap() async {
    if (!_controller.finalPhaseEligible) {
  ...
  Future<void> _openGeneralQuiz() async {
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => GeneralQuizScreen(
          college: widget.college,
          specialization: widget.specialization,
        ),
      ),
  ...
  Widget _buildContent(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        ...
        child: Row(
          children: [
            if (widget.onChangeCareer != null) ...[
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: widget.onChangeCareer,
                  icon: const Icon(Icons.swap_horiz_rounded),
                  label: Text(t(context, 'Change Career', 'تغيير المسار')),
            ...
            children: [
              ...
              CustomScrollView(
                slivers: [
                  ...
                  SliverToBoxAdapter(
                    child: _GeneralQuizCard(
                      isPassed: _controller.generalQuizPassed,
                      onTap: _openGeneralQuiz,
                    ),
                  ),

                  SliverToBoxAdapter(
                    child: FinalPhaseGateCard(
                      isUnlocked: _controller.finalPhaseEligible,
                      unlockedByGeneralQuiz:
                          _controller.generalQuizPassed &&
                          !_controller.phase1And2Completed,
...
class _GeneralQuizCard extends StatelessWidget {
  final bool isPassed;
  final VoidCallback onTap;

  const _GeneralQuizCard({required this.isPassed, required this.onTap});
      ...
      child: InkWell(
        ...
        child: Ink(
          ...
          child: Row(
            children: [
              Icon(
                isPassed ? Icons.verified_outlined : Icons.quiz_outlined,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      isPassed
                          ? 'Passed. Final Phase can be unlocked without marking all subjects completed.'
                          : 'Take the major-level quiz to prove overall readiness.',
