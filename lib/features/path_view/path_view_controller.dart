...
class PathViewController extends ChangeNotifier {
  PathViewController({required this.college, required this.specialization})
    : _repository = SubjectRepository(
        localDataSource: SubjectLocalDataSource(),
      ),
      _progressService = ProgressService(),
      _attemptLimitService = QuizAttemptLimitService(),
      _deadlineService = SubjectDeadlineService(),
      _generalQuizStorageService = GeneralQuizStorageService();
  ...
  final GeneralQuizStorageService _generalQuizStorageService;
  ...
  bool generalQuizPassed = false;
  bool finalPhaseEligible = false;
  ...
  Future<void> loadPath() async {
    ...
    try {
      final subjects = await _repository.getSubjectsByCollegeAndSpecialization(
        college: college,
        specialization: specialization,
      );

      final ordered = PathGenerator.generateOrderedPath(subjects);
      final completed = await _progressService.getCompletedSubjects(
        specialization,
      );

      final lastCode = await _progressService.getLastOpenedSubject(
        specialization,
      );

      final generalQuizStatus = await _generalQuizStorageService.loadStatus(
        specialization,
      );

      allSubjects = ordered;
      phase1Subjects = ordered
          .where((s) => s.phase == 1)
          .toList(growable: false);
      phase2Subjects = ordered
          .where((s) => s.phase == 2)
          .toList(growable: false);
      completedSubjects = completed;
      generalQuizPassed = generalQuizStatus.finalPhaseEligibleByGeneralQuiz;
      ...
      selectedSubject = restored ?? (ordered.isNotEmpty ? ordered.first : null);
  ...
  Future<void> openSubject(Subject subject) async {
    ...
    await _progressService.setLastOpenedSubject(specialization, subject.code);
  ...
  Future<void> markCompleted(Subject subject) async {
    ...
    await _deadlineService.recordCompletion(
      '${specialization.trim()}::${subject.code.trim()}',
    );

    completedSubjects = await _progressService.getCompletedSubjects(
      specialization,
    );
  ...
  void _recomputeDerived() {
    ...
    finalPhaseEligible = phase1And2Completed || generalQuizPassed;
