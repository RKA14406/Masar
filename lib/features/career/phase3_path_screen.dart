...
class _Phase3PathScreenState extends State<Phase3PathScreen> {
  ...
  final SubjectDeadlineService _deadlineService = SubjectDeadlineService();
  ...
  Future<void> _completeNodeFromLongPress(Subject subject) async {
    ...
    if (_isCompleted(subject)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(l10n.phase3_already_completed(subject.name))),
      );
      return;
    }

    if (!_isUnlocked(subject)) {
      final names = _missingPrerequisites(
        subject,
      ).map((e) => e.name).join(', ');
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(l10n.phase3_locked_first(names))));
      return;
    }
    ...
    await _deadlineService.recordCompletion(
      '${widget.specialization.trim()}::${subject.code.trim()}',
    );
    await _loadScreen();
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(l10n.phase3_marked_completed(subject.name))),
    );
  }
  ...
  Future<QuizAttemptResult?> _attemptCompletionQuiz(Subject subject) async {
    ...
    if (!canStart) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(l10n.phase3_attempt_limit(subject.name))),
      );
      return null;
    }
    ...
    if (_phase3Nodes.isEmpty) {
      return Scaffold(
        ...
        body: Center(
          child: Padding(
            padding: const EdgeInsets.all(20),
            child: Text(l10n.phase3_empty, textAlign: TextAlign.center),
          ),
        ),
      );
    }
