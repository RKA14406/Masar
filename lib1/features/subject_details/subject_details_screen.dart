...
class _SubjectDetailsScreenState extends State<SubjectDetailsScreen> {
  ...
  Future<void> _setDeadlineMonths(int months) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Learning target set to $months month(s).')),
    );
  }
  ...
  Future<void> toggleCompletion(bool value) async {
    ...
    if (isCompleted) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${widget.subject.name} is already completed.')),
      );
      return;
    }
  ...
  Future<void> _showDeveloperSkipDialog() async {
    ...
    if (result != _developerBypassCode) {
      messenger.showSnackBar(
        const SnackBar(content: Text('Wrong developer code.')),
      );
      return;
    }
  ...
  String _deadlineRemainingText() {
    final item = deadline;
    if (item == null) return 'No learning target set.';
    final remaining = item.targetDate.difference(DateTime.now()).inDays;
    if (remaining >= 0) return '$remaining day(s)';
    return '${remaining.abs()} day(s) late';
  }

  String _deadlineStatusText() {
    final item = deadline;
    if (item == null) return 'No deadline set';
    if (item.completedOnTime) return 'Completed on time (+10 bonus)';
    if (item.completedLate) return 'Completed late (+0 bonus)';
    if (DateTime.now().isAfter(item.targetDate)) return 'Late';
    return 'On track';
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 14),
            Text(
              'Target: ${target == null ? '-' : '${target.targetMonths} month(s)'}',
            ),
            const SizedBox(height: 6),
            Text('Remaining: ${_deadlineRemainingText()}'),
            const SizedBox(height: 6),
            Text(
              'Status: ${_deadlineStatusText()}',
              style: TextStyle(
                color: _deadlineStatusColor(),
            ...
            const SizedBox(height: 6),
            Text('Deadline bonus: ${target?.deadlineBonus ?? 0}'),
          ],
        ),
      ),
    );
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 8),
                          Text(
                            'Specialization: ${isArabic && subject.specializationAr != null ? subject.specializationAr : subject.specialization}',
                          ),
                        ],
                      ),
                    ),
